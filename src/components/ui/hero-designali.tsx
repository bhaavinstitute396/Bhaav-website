...
const TypeWriter = ({ strings }: TypeWriterProps) => {
  ...
  return (
    <span className="inline-flex items-center">
      ...
      <span className="animate-pulse ml-0.5 font-light text-cyan-400">|</span>
    </span>
  );
};
...
function ShineBorder({
  ...
  return (
    <div
      ...
      className={cn(
        "relative grid h-full w-full place-items-center rounded-3xl bg-white p-3 text-black dark:bg-black dark:text-white cursor-pointer",
        className,
      )}
      {...props}
    >
      <div
        ...
        className={`pointer-events-none before:pointer-events-none before:bg-shine-size before:absolute before:inset-0 before:aspect-square before:size-full before:rounded-3xl before:p-[--border-width] before:will-change-[background-position] before:content-[""] before:![-webkit-mask-composite:xor] before:[background-image:--background-radial-gradient] before:[background-size:300%_300%] before:![mask-composite:exclude] before:[mask:--mask-linear-gradient] motion-safe:before:animate-[shine-pulse_var(--shine-pulse-duration)_infinite_linear]`}
      ></div>
      {children}
    </div>
  );
